...
class PairRDDFunctions[K, V](self: RDD[(K, V)])
  ...
  with Serializable
{
  /**该方法用于将多个RDD[<K,V>]合并成[K,Seq[V]=C], 所以需要用到
    * createCombiner:在C不存在时，通过V生成C;
    * mergeValue:将V添加到C的后面；
    * mergeCombiners：将两个C合并成一个；
    * partitioner：合并的话肯定要shuffle，所以要提供分区方法
    * mapSideCombine：类似于MR中的Combiner
    * serializer：shuffle过程需要序列化
